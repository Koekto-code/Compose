#include "../../include/Compose/Lexer.h"

#include <stdio.h>
#include <assert.h>
#include <string.h>
#include <stdlib.h>

static int isWhitespace(char ch)
{
	switch (ch)
	{
		case ' ':
		case '\t':
		case '\r':
		case '\n':
		case '\v':
		case '\f':
			return 1;
		default:
			return 0;
	}
}

static int isOperChar(char ch)
{
	switch (ch)
	{
		case '=':
		case '!':
		case '*':
		case '/':
		case '+':
		case '-':
		case '&':
		case '^':
		case '~':
		case '%':
		case '.':
		case ',':
			return 1;
		default:
			return 0;
	}
}

static int isControlChar(char ch)
{
	switch (ch)
	{
		case '(':
		case ')':
		case '{':
		case '}':
		case ';':
		case ':':
			return 1;
		default:
			return 0;
	}
}

static int isDigit(char ch)
{
	return ch >= '0' && ch <= '9';
}

static int isLetter(char ch)
{
	return (ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z');
}

typedef enum
{
	IsUndefined   = 0x0,
	IsWhitespace  = 0x1,
	IsOperChar    = 0x2,
	IsControlChar = 0x4,
	IsDigit       = 0x8,
	IsLetter      = 0x10
}
SymbolState;

static SymbolState getSymbolState(char ch)
{
	SymbolState flags = IsUndefined;
	flags |= isWhitespace(ch) * IsWhitespace;
	if (!flags)
	{
		flags |= isOperChar(ch) * IsOperChar;
		flags |= isControlChar(ch) * IsControlChar;
		flags |= isDigit(ch) * IsDigit;
		flags |= isLetter(ch) * IsLetter;
	}
	return flags;
}

// compares null-terminated string with another that may be not null-terminated,
// up to maxlen symbols. if cstr has more symbols, strings aren't considered equal.
static int strCompare(const char *cstr, const char *s, unsigned maxlen)
{
	for (unsigned i = 0; i != maxlen; ++i)
		if (!cstr[i] || cstr[i] != s[i])
			return 0;

	return !cstr[maxlen];
}

// yet one function to determine if cstr is s or part of s. returns end position if true
static unsigned strPartCompare(const char *cstr, const char *s, unsigned maxlen)
{
	for (unsigned i = 0; i != maxlen; ++i)
	{
		if (!cstr[i])
			return i;

		if (cstr[i] != s[i])
			return 0;
	}
	return cstr[maxlen] ? 0 : maxlen;
}

static const char *mlControls[MODULAR_CONTROLS_NUM] =
{
	"return",
	"function",
	"{",
	"}",
	"(",
	")",
	"if",
	"else",
	"break",
	"continue",
	"for",
	";"
};

static CmTokenData mlControlCodes[MODULAR_CONTROLS_NUM] =
{
	Tdt_Control_Return,
	Tdt_Control_Function,
	Tdt_Control_ScopeBeg,
	Tdt_Control_ScopeEnd,
	Tdt_Control_ParBeg,
	Tdt_Control_ParEnd,
	Tdt_Control_If,
	Tdt_Control_Else,
	Tdt_Control_Break,
	Tdt_Control_Continue,
	Tdt_Control_For,
	Tdt_Control_Semicolon
};

static const char *mlOperators[MODULAR_OPERATORS_NUM] = {
	"=",
	"==",
	"!=",
	">",
	">=",
	"<",
	"<=",
	"+",
	"-",
	"*",
	"/",
	".",
	":",
	","
};

static CmTokenData mlOpCodes[MODULAR_OPERATORS_NUM] =
{
	Tdt_Oper_Equals,
	Tdt_Oper_IsEqual,
	Tdt_Oper_IsNEqual,
	Tdt_Oper_IsGreater,
	Tdt_Oper_IsGreaterEqual,
	Tdt_Oper_IsLess,
	Tdt_Oper_IsLessEqual,
	Tdt_Oper_Add,
	Tdt_Oper_Sub,
	Tdt_Oper_Mul,
	Tdt_Oper_Div,
	Tdt_Oper_Dot,
	Tdt_Oper_Colon,
	Tdt_Oper_Comma
};

const char *CmOperString(CmTokenData data)
{
	for (int i = 0; i != MODULAR_OPERATORS_NUM; ++i)
		if (mlOpCodes[i] == data)
			return mlOperators[i];
	return NULL;
}

const char *CmControlString(CmTokenData data)
{
	for (int i = 0; i != MODULAR_CONTROLS_NUM; ++i)
		if (mlControlCodes[i] == data)
			return mlControls[i];
	return NULL;
}

typedef struct
{
	// current line number
	int line;
	
	// address of first symbol in current line
	const char *linebeg; 
	
	// currently parsed token
	CmToken token;
	// const char *tokenbeg;
	// const char *tokenend;
	
	// fileend - filebeg == filesize
	const char *filebeg;
	const char *fileend;
	const char *filename;

	int errors;
}
lexerFlowData;

/* @deprecated
CmToken CmRecognizeToken(const char *beg, const char *end)
{
	assert(end - beg > 0);
	CmToken token = { Ttp_Undefined, Tdt_Undefined, 0, beg, end };
	
	if (end - beg >= 4)
	{
		if (beg[0] == '<' && beg[1] == '"' && end[-2] == '"' && end[-1] == '>')
		{
			token.type = Ttp_Literal;
			token.data = Tdt_Literal_String;
			return token;
		}
	}
	if (isDigit(beg[0]))
	{
		token.type = Ttp_Literal;
		int has_point = 0;
		for (const char *s = beg + 1; s != end; ++s)
		{
			if (!isDigit(*s))
			{
				if (*s == '.' && !has_point)
					has_point = 1;
				else return token;
			}
		}
		if (has_point)
			token.data = Tdt_Literal_Float;
		else token.data = Tdt_Literal_Int;
		
		return token;
	}
	if (isOperChar(beg[0]))
	{
		token.type = Ttp_Oper;
		for (int op = 0; op < MODULAR_OPERATORS_NUM; ++op)
		{
			if (!strncmp(beg, mlOperators[op], end - beg))
			{
				token.data = mlOpCodes[op];
				return token;
			}
		}
		return token;
	}
	
	for (int i = 0; i != MODULAR_CONTROLS_NUM; ++i)
	{
		if (!strncmp(beg, mlControls[i], end - beg))
		{
			token.type = Ttp_Control;
			token.data = mlControlCodes[i];
			return token;
		}
	}
	
	if (!isLetter(*beg))
		return token;
	
	for (const char *s = beg + 1; s != end; ++s)
		if (!isLetter(*s) && !isDigit(*s))
			return token;
	
	token.type = Ttp_Var;
	// token.data to be determined later
	return token;
}
*/

static const char *skipWhitespace(lexerFlowData *flow, const char *s)
{
	if (*s == '\n')
	{
		++s;
		if (s + 1 != flow->fileend && *s == '\r')
			++s;

		++flow->line;
		flow->linebeg = s;
		return s;
	}
	if (*s == '\r')
	{
		++s;
		if (s + 1 != flow->fileend && *s == '\n')
			++s;

		++flow->line;
		flow->linebeg = s;
		return s;
	}
	return s + 1;
}

static int findNextToken (lexerFlowData *flow)
{
	flow->token.type = Ttp_Undefined;
	flow->token.data = Tdt_Undefined;
	
	const char *s = flow->token.beg /* +1 */;

	// dont forget about line continuation inside tokens
	// (like string <" "> tokens)
	while (s != flow->token.end)
		s = skipWhitespace(flow, s);

	while (s != flow->fileend && isWhitespace(*s))
		s = skipWhitespace(flow, s);

	flow->token.beg = flow->token.end = s;
	if (s == flow->fileend)
		return 0;

	SymbolState st = getSymbolState(*s);
	
	// guessing which literal type it is
	if (st & IsDigit)
		flow->token.type |= Ttp_Literal;
	if (st & IsOperChar)
		flow->token.type |= Ttp_Oper | Ttp_Literal;
	if (st & IsControlChar)
		flow->token.type |= Ttp_Control;
	if (st & IsLetter)
		flow->token.type |= Ttp_Control | Ttp_Var;

	
	// don't report errors if the token type is undefined
	// it's job of parseToken
	return 1;
}

// this function does 2 things:
// 1. finding token end
// 2. recognizing the token
static int parseToken (lexerFlowData *flow)
{
	assert(flow->token.beg < flow->fileend);
	const char *const beg = flow->token.beg;

	if (flow->token.type & Ttp_Literal)
	{
		// guessing string literal
		if (flow->fileend - beg >= 2)
		{
			if (beg[0] == '<' && beg[1] == '"')
			{
				const char *s;
				for (s = beg + 2; s != flow->fileend - 1; ++s)
					if (s[0] == '"' && s[1] == '>')
						break;
				
				if (s != flow->fileend)
					s += 2;
				else
				{
					++flow->errors;
					printf (
						"%s:%d:%d: error: \"> expected\n",
						flow->filename, flow->line,
						(int)(s - flow->linebeg)
					);
					return 0;
				}

				flow->token.type = Ttp_Literal;
				flow->token.data = Tdt_Literal_String;
				flow->token.end = s;
				return 1;
			}
		}

		int has_point = 0;
		const char *s;
		for (s = beg; s != flow->fileend; ++s)
		{
			if (!isDigit(*s))
			{
				if (*s == '.' && !has_point)
					has_point = 1;
				else break;
			}
		}
		if (s != beg)
		{
			flow->token.type = Ttp_Literal;
			if (has_point)
				flow->token.data = Tdt_Literal_Float;
			else flow->token.data = Tdt_Literal_Int;
			flow->token.end = s;
			return 1;
		}
	}

	if (flow->token.type & Ttp_Oper)
	{
		const char *s;
		for (s = beg + 1; s != flow->fileend; ++s)
			if (!isOperChar(*s))
				break;

		for (int i = 0; i != MODULAR_OPERATORS_NUM; ++i)
		{
			if (strCompare(mlOperators[i], beg, s - beg))
			{
				flow->token.type = Ttp_Oper;
				flow->token.data = mlOpCodes[i];
				flow->token.end = s;
				return 1;
			}
		}
	}

	if (flow->token.type & Ttp_Control)
	{
		const char *s = beg;
		SymbolState st = getSymbolState(*s);

		for (++s; s != flow->fileend; ++s)
			if (getSymbolState(*s) != st)
				break;

		if (st & IsControlChar)
		{
			for (int i = 0; i != MODULAR_CONTROLS_NUM; ++i)
			{
				unsigned ctlend;
				if (( ctlend = strPartCompare(mlControls[i], beg, s - beg) ))
				{
					flow->token.type = Ttp_Control;
					flow->token.data = mlControlCodes[i];
					flow->token.end = beg + ctlend;
					return 1;
				}
			}
		}
		else
		{
			for (int i = 0; i != MODULAR_CONTROLS_NUM; ++i)
			{
				if (strCompare(mlControls[i], beg, s - beg))
				{
					flow->token.type = Ttp_Control;
					flow->token.data = mlControlCodes[i];
					flow->token.end = s;
					return 1;
				}
			}
		}
	}

	if (flow->token.type & Ttp_Var)
	{
		assert(isLetter(*beg));
		const char *s;
		for (s = beg + 1; s != flow->fileend; ++s)
			if (!isLetter(*s) && !isDigit(*s))
				break;

		flow->token.type = Ttp_Var;
		flow->token.end = s;
		return 1;
	}

	++flow->errors;
	++flow->token.end;
	printf (
		"%s:%d:%d: error: unknown token\n",
		flow->filename, flow->line,
		(int)(beg - flow->linebeg)
	);
	return 0;
}

CmLexerData CmLexerProcessFile(const char *filename)
{
	CmLexerData result = { CmVectorCreate(sizeof(CmToken)), 0 };
	lexerFlowData flow;
	flow.filename = filename;
	
	FILE *fin = fopen(filename, "rb");
	if (fin == NULL)
		return result;
	
	if (fseek(fin, 0, SEEK_END) != 0)
		goto fexit;
	
	int fsize = (int)ftell(fin);
	if (fsize == 0)
		goto fexit;
	
	if (fseek(fin, 0, SEEK_SET) != 0)
		goto fexit;
	
	char *buffer = malloc(fsize);
	if (buffer == NULL)
		goto fexit;
	
	int rd = fread(buffer, 1, fsize, fin);
	if (rd != fsize)
		goto fexit;
	
	flow.errors = 0;
	flow.token.beg = flow.token.end = buffer;
	flow.fileend = buffer + fsize;
	flow.line = 1;
	
	while (1)
	{
		if (!findNextToken(&flow))
			break;
		
		if (parseToken(&flow))
			CmVectorPushBack(&result.tokens, &flow.token);
	}
	result.success = 1;
	result.syntaxErrors = flow.errors;

	return result;
	
fexit:
	fclose(fin);
	return result;
}
